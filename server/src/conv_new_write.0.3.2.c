...
int save_stat(int level)
{
   ...
   CREATE(numeri_urne, long int, alloc, TYPE_LONG);
...
#if 0
struct urna_prop *read_prop(unsigned long progressivo, int modo)
{
   FILE *fp;
   int i, j;
   char duepunti[]="A";
  ...
  slots=1; 
   ...
   while(!feof(fp)) {
      if(fread(&matricola, sizeof(long), 1, fp) != 1)
         break;

      if(fread(duepunti, sizeof(char), 1, fp) != 1)
         break;
      ...
      if(num < 0 || num > MAX_VOCI)
         break;

      if(fread(acapo, sizeof(char), 1, fp) != 1)
         break;
      ...
      for(i = 0; i < num; i++) {
         if(fread(&prop_i, sizeof(int), 1, fp) != 1)
            break;
         ...
         if(fread(&lent, sizeof(size_t), 1, fp) != 1)
            break;
         ...
         CREATE(*(testo+j), char, lent+1, TYPE_CHAR);

         if(fread(*(testo+j), sizeof(char), lent, fp) != lent)
            break;
         j++;
       if(j>slots*LEN_SLOTS){
         slots++;
            RECREATE(testo, char *, slots*LEN_SLOTS);
       }
