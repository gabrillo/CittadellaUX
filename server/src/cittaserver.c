...
int citta_reboot = 1;          /* riavvia il server dopo lo shutdown      */
...
time_t force_script_check(void);
int force_script_delete(void);
...
void salva_dati_server(void)
{
	...
	hh = fwrite((struct dati_server *) &dati_server,
		    sizeof(struct dati_server), 1, fp);
