...
int new_msg(struct sessione *t, struct room *r);
...
static int msg_send(struct sessione *t, struct room *room, int i, int reply);
...
int save_post(struct sessione *t, char *arg)
{
	...
	long fmnum, msgnum, msgpos, rcpt, i, flags = 0, reply = 0, matricola;
	int sysm = 0;
        size_t hdrlen;
	struct room *room;
	struct sessione *s;
...
/*
 * Restituisce 1 se ci sono nuovi messaggi in r per t, altrimenti 0.
 * Se la room r e' dinamica, restituisce false
 */
int new_msg(struct sessione *t, struct room *r)
{
        if (r->data->type != ROOM_DYNAMICAL)
                return ((t->fullroom[r->pos] < get_highmsg_room(t,r)) ? 1 : 0);
        return FALSE;
}
