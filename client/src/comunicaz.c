...
static struct xmsg *xlog;
static struct xmsg xmsg_tmp;            /* Dati temporanei sull'X in arrivo */
...
static void xlog_xmsg(struct xmsg *xmsg, struct text *txt, const char *name,
                      struct name_list *dest, int ore, int min, int type);
static void xlog_print(int num);
static void xlog_print_all(void);
static void xlog_header(struct xmsg x);
static void xlog_list(void);
#ifdef LOCAL
static void xlog_dump(struct xmsg *xmsg, int autolog);
static void xlog_header_dump(FILE *fp, struct xmsg x);
#endif
...
void express(char *rcpt)
{
	...
	if (uflags[0] & UT_LINE_EDT_X) {
		if (get_textl_cursor(txt,78,serverinfo.maxlineebx-2,FALSE) > 0)
			send_text(txt);
...
void ascolta_chat (void)
{
        ...
        putchar('\n');
        if (chat) {
                cml_printf(sesso ? _("Sei gi&agrave; connessa al canale #%d della chat. Vuoi cambiare canale? ") : _("Sei gi&agrave; connesso al canale #%d della chat. Vuoi cambiare canale? "), chat);
                if (si_no() == 'n')
                        return;
        }
...
void chat_dump(char *nome, struct text *txt, int mode)
{
	FILE *fp;
	char *str;
...
void xlog_init(void)
{
	CREATE(xlog, struct xmsg, MAX_XLOG, 0);
	/*
	int i;
