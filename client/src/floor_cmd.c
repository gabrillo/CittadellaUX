...
void floor_info(int det)
{
	...
	long numpost, numroom, ct, mt, pt, flags;
...
void floor_edit(int mode)
{
	...
	if (buf[0] == '2') {
		...
		wlvl = new_int_def(_("Livello minimo per scrittura"), wlvl);
                ...
                */
		if (new_si_no_def(_("Subject"), flags & FF_SUBJECT))
			newflags |= FF_SUBJECT;
		if (new_si_no_def(_("Reply"), flags & FF_REPLY))
			newflags |= FF_REPLY;
		if (new_si_no_def(_("Invito"), flags & FF_INVITO))
			newflags |= FF_INVITO;
		if (newflags & FF_INVITO) {
			...
			if (new_si_no_def(_("Guess Room"), flags & FF_GUESS))
				newflags |= FF_GUESS;
		}
		if (new_si_no_def(_("Messaggi anonimi"), flags & FF_ANONYMOUS))
			newflags |= FF_ANONYMOUS;
		if (!(newflags & FF_ANONYMOUS))
			if (new_si_no_def(_("Post anonimo facoltativo"),
                                          flags & FF_ASKANONYM))
                                newflags |= FF_ASKANONYM;
		if (new_si_no_def(_("Attivata"), flags & FF_ENABLED))
			newflags |= FF_ENABLED;
		flags = newflags;
		if (mode == 1) {
