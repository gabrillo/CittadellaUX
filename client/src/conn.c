...
void parse_opt(int argc, char **argv, char **rcfile, int *no_rc);
...
void serv_write(const char *buf, int bytes, int progressbar);
static int serv_buffer_has_input(void);
int elabora_input(void);

int prendi_da_coda(struct coda_testo *coda, char **dest);
int prendi_da_coda_t(struct coda_testo *coda, char **dest, int mode);
...
#if 0
#ifdef _GNU_SOURCE /* Questo e' il modo piu' pulito, ma vasprintf() e'
	            * solo nello standard GNU e non di default in cygwin. */
        char *str;
...
{
        int ret, prog_step, j;
        int progress, total, questo_giro;

        if (progressbar) {
                progress = 1;
                total = bytes;
                prog_step = total / 10;
                questo_giro = prog_step;
                while (bytes > 0) {
                        if (prog_step > bytes)
                                prog_step = bytes;
