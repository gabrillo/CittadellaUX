...
static int auto_validazione(void);
 ...
 * Setta la var ospite=1 . Restituisce 1 se e' andato tutto bene.
 */
static int login_ospite(void)
{
        char buf[LBUF];
...
void user_config(int type)
{
	...
	if ((type == LOGIN_OSPITE) || (type == LOGIN_NUOVO)) {
		uflags[1] |= UT_ANSI;
		if (!new_si_no_def(_("\nIl tuo terminale &egrave; in grado di visualizzare i colori ansi?"), uflags[1] & UT_ANSI)) {
			if (!(uflags[1] & UT_ANSI) &&
