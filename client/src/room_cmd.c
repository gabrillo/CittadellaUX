...
void room_goto(int mode, int gotonext, const char *destroom);
void room_create(void);
void room_edit(int mode);
void room_delete(void);
void room_info(int det, int prof);
...
void room_list (void)
{
        ...
        char nm[ROOMNAMELEN];
	int rlvl, wlvl, riga;
        long num, fmnum, maxmsg, ora, flags, msgnum;
        struct tm *tmst;
...
void room_edit(int mode)
{
	...
	int tipo, rlvl, wlvl;
	long fmnum, maxmsg, flags, newflags = 0;
	...
	if (buf[0] == '2') {
		...
		wlvl = new_int_def(_("Livello minimo per scrittura"), wlvl);
                ...
                */
		if (new_si_no_def(_("Subject"), flags & RF_SUBJECT))
			newflags |= RF_SUBJECT;
		if (new_si_no_def(_("Reply"), flags & RF_REPLY))
			newflags |= RF_REPLY;
		if (new_si_no_def(_("Invito"), flags & RF_INVITO))
			newflags |= RF_INVITO;
		if (newflags & RF_INVITO) {
			if(new_si_no_def(_("Invito automatico"),
					 flags & RF_AUTOINV))
				newflags |= RF_AUTOINV;			
		} else if (new_si_no_def(_("Guess Room"), flags & RF_GUESS))
			newflags |= RF_GUESS;
		if (new_si_no_def(_("Upload/Download files"),
                                  flags & RF_DIRECTORY))
                        newflags |= RF_DIRECTORY;
		if (new_si_no_def(_("Messaggi anonimi"), flags & RF_ANONYMOUS))
			newflags |= RF_ANONYMOUS;
		if (!(newflags & RF_ANONYMOUS))
			if (new_si_no_def(_("Post anonimo facoltativo"),
                                          flags & RF_ASKANONYM))
                                newflags |= RF_ASKANONYM;
		if (new_si_no_def(_("Messaggi con spoiler"), flags&RF_SPOILER))
			newflags |= RF_SPOILER;
		...
		if (new_si_no_def(_("Ricerca nei messaggi"),
				  flags & RF_FIND))
			newflags |= RF_FIND;
		if (new_si_no_def(_("Pubblica su Web"), flags & RF_CITTAWEB))
			newflags |= RF_CITTAWEB;
		if (new_si_no_def(_("Attivata"), flags & RF_ENABLED))
			newflags |= RF_ENABLED;
		flags = newflags;
		if (mode == 1) {
...
void room_info(int det, int prof)
{
	...
	long local, ct, mt, pt, flags;
        int blogroom = FALSE; /* TODO sistemare */
