...
int get_textl_cursor(struct text *txt, int max, int nlines, int chat);
...
char * getline_col(int max, int maiuscole, int special);
int get_text_col(struct text *txt, long max_linee, int max_col, int abort);
int gl_extchar(int status);

static int getline_wrap(char *str, int max, int maiuscole, int special,
			int wrap, int pos);
	...
	str[0] = '\0';
		...
		if (pos == 0) {
			...
			if (riga == 0)
				...
				return riga+1;
		} else if (abort && !strcmp(str, "ABORT")) {
			prompt_curr = prompt_tmp;
			return -1;
		}
		str[pos] = '\n';
...
static int getline_col_wrap(int **str0, int **col0, int max, int maiuscole,
			    int special, int wrap, int pos);

inline char * getline_col(int max, int maiuscole, int special)
{
	int *str;
	int *col;
	char *out;
...
static int getline_col_wrap(int **str0, int **col0, int max,
			    int maiuscole, int special, int wrap, int pos)
{
...
int get_text_col(struct text *txt, long max_linee, int max_col, int abortp)
{
