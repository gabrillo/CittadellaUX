...
int save_userconfig(int update);
int toggle_uflag(int slot, long flag);
...
char edit_uc(char mode)
{
	...
	case 1:
		...
		if (new_si_no_def(_(" Usa prompt che spariscono    "),
				  uflags[1] & UT_DISAPPEAR))
			flag |= UT_DISAPPEAR;
		if (new_si_no_def(_(" Pausa a ogni schermata       "),
				  uflags[1] & UT_PAGINATOR))
			flag |= UT_PAGINATOR;
	...
	case 2:
		...
		printf(_("\nDati personali visibili a tutti:\n"));
		setcolor(C_CONFIG_B);
		if (new_si_no_def(_(" Nome reale "), uflags[2] & UT_VNOME))
			flag |= UT_VNOME;
		if (new_si_no_def(_(" Indirizzo  "), uflags[2] & UT_VADDR))
			flag |= UT_VADDR;
		if (new_si_no_def(_(" N. telefono"), uflags[2] & UT_VTEL))
			flag |= UT_VTEL;
		if (new_si_no_def(_(" Email      "), uflags[2] & UT_VEMAIL))
			flag |= UT_VEMAIL;
		if (new_si_no_def(_(" Home page  "), uflags[2] & UT_VURL))
			flag |= UT_VURL;
		if (new_si_no_def(_(" Sesso      "), uflags[2] & UT_VSEX))
			flag |= UT_VSEX;
...
int toggle_uflag(int slot, long flag)
{
	...
	print_on_off(uflags[slot] & flag);
	save_userconfig(TRUE);
	return ((uflags[slot] & flag) ? TRUE : FALSE);
}
