...
static int tcnewprompt_def(const char *prompt, char *def, char *str, int len,
                           int mode);
static int cerca_completamento(char *cmp, char *buf, int num, int mode);
static int check_recipient(struct name_list *nl, char *caio, int status);
 ...
 */
int get_roomname(const char *prompt, char *str, int blog)
{
        int ret;
        int mode;
 ...
 */
int get_recipient(char *caio, int mode)
{
...
#else
		sprintf(format, "%%%us#%%d: ", prompt_len-2);
#endif
